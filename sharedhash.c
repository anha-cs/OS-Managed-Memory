...
// New: Added new functions needed for threaded mode
...
// New: Simple pointer for thread/single-process mode
...
sem_t* mLock;
pthread_mutex_t mLock_thread; // New: Mutex for thread synchronization 
int use_multiprocess = 0; 
int use_multithread = 0; // New: Add a flag for the new mode - multi-thread mode
...
void *init_umem(void) {
    ...
    // New: Multi-Thread (Used for -t) Mode and Single-thread mode without the mutex
    else {
        if (use_multithread) {
            // New: Initialize mutex for thread synchronization 
    ...
    // New: Use malloc() for the heap base (memory is automatically shared by threads in thread mode)
    ...
    // New: Initialize the free list (using the new simple pointer)
 ...
 */
 ...
 // New: Macro to conditionally access the free list head
...
void *umalloc(size_t size) {
    ...
    // New: thread lock
    else if (use_multithread) pthread_mutex_lock(&mLock_thread);
    ...
    // New: thread unlock
    else if (use_multithread) pthread_mutex_unlock(&mLock_thread);
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        fprintf(stderr, "Usage: %s <file> [-m|-t]\n", argv[0]); // Introduce new mode - thread mode
        return 1;
    }

    const char *filename = argv[1];
    use_multiprocess = (argc >= 3 && strcmp(argv[2], "-m") == 0);
    use_multithread = (argc >= 3 && strcmp(argv[2], "-t") == 0); // New check for thread mode

    init_umem();

    if (use_multiprocess)
        return run_multi(filename);
    else if (use_multithread) // New: Thread mode
...
// New: Set a safe limit for concurrent threads. 32 or 64 is safe for a 2MB heap
...
// New: helper functions for thread mode
void *thread_worker(void *arg) {
